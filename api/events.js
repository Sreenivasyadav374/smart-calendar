  ...
  try {
    ...
    switch (req.method) {
      ...
      case 'POST':
        ...
        const createdEvent = await collection.findOne({ _id: insertResult.insertedId });
        res.status(201).json({
          ...createdEvent,
          id: createdEvent.id || createdEvent._id.toString(),
        });
        break;

      case 'PUT':
        const eventId = req.query.id || req.url.split('/').pop();
        ...
        res.status(200).json({
          ...updateResult.value,
          id: updateResult.value.id || updateResult.value._id.toString(),
        });
        break;
